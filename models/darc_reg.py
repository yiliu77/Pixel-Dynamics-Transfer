...
from architectures.classifiers import SAClassifier, SASClassifier


class DARCReg(ContSAC):
    def __init__(self, policy_config, value_config, sa_config, sas_config, source_env, target_env,
        ...
        super(DARCReg, self).__init__(policy_config, value_config, source_env, log_dir,
        ...
        self.sa_classifier = SAClassifier(state_dim, action_dim, sa_config)
        self.sa_classifier_opt = Adam(learning_rate=lr)
        self.sas_adv_classifier = SASClassifier(state_dim, action_dim, sas_config)
    ...
    def train_step(self, s_states, s_actions, s_rewards, s_next_states, s_done_masks, *args):
        ...
        sa_logits = self.sa_classifier((s_states, s_actions))
        sas_logits = self.sas_adv_classifier((s_states, s_actions, s_next_states))
        ...
        train_info = super(DARCReg, self).train_step(s_states, s_actions, s_rewards, s_next_states, s_done_masks)

        with tf.GradientTape(persistent=True) as tape:
            s_sa_logits = self.sa_classifier((s_states, s_actions))
            s_sas_logits = self.sas_adv_classifier((s_states, s_actions, s_next_states))
            t_sa_logits = self.sa_classifier((t_states, t_actions))
            t_sas_logits = self.sas_adv_classifier((t_states, t_actions, t_next_states))
    ...
    def load_embedding(self, folder_name):
        path = 'saved_weights/' + folder_name
        self.twin_q.embedding.load_weights(path + '/embed')
        self.policy.embedding.load_weights(path + '/embed')
