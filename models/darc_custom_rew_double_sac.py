...
class DARCCustomRew2:
    ...
    def train_step(self, s_states, s_actions, s_rewards, s_next_states, s_done_masks, *args):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            total_states = tf.concat([s_states, t_states], axis=0)
            total_next_states = tf.concat([s_next_states, t_next_states], axis=0)
            total_states_embed = tf.concat([s_states_embed, t_states_embed], axis=0)
            total_next_states_embed = tf.concat([s_next_state_embed, t_next_state_embed], axis=0)
            total_rewards = tf.concat([s_rewards, t_rewards], axis=0)
            total_actions = tf.concat([s_actions, t_actions], axis=0)
            total_done_masks = tf.concat([s_done_masks, t_done_masks], axis=0)

            next_actions, next_log_probs, _ = self.policy.sample(total_next_states_embed)
            next_q = self.target_twin_q([total_next_states, next_actions])[0]
            v = next_q - self.alpha * next_log_probs
            expected_q = tf.stop_gradient(total_rewards + total_done_masks * self.gamma * v)

            # Q backprop
            q_val, pred_q1, pred_q2 = self.twin_q([total_states, total_actions])
            q_loss = losses.mean_squared_error(pred_q1, expected_q) + losses.mean_squared_error(pred_q2, expected_q)
            q_loss = tf.reduce_mean(q_loss)

            # Policy backprop
            s_action, s_log_prob, _ = self.policy.sample(total_states_embed)
            policy_loss = self.alpha * s_log_prob - self.twin_q([total_states, s_action])[0]
